...
from utils import Method, Metric, compute_metric, make_grid


@st.cache_data
def plot_metric(data: dict[str, tuple[np.ndarray, np.ndarray]], metric: Metric) -> plt.Figure:
    score = compute_metric(data, metric)
    fig, ax = plt.subplots()
...
def render_metrics() -> None:
    ...
    for dataset_name, dataset in datasets.items():
        ...
        for metric_idx, metric in enumerate(Metric):
            grid[0][metric_idx + 1].pyplot(plot_metric(dataset, metric))